...
namespace TapTap.AntiAddiction.Internal
{
    internal static class Network {
        ...
        internal static async Task<AntiAddictionConfigResult> FetchConfig() {
            string path;
            if (!enableTestMode) {
                path = $"anti-addiction/v1/{TapTapAntiAddictionManager.AntiAddictionConfig.regionStr}/clients/{gameId}/configuration";
            }
            else {
                path = $"anti-addiction/v1/clients/{gameId}/configuration";
            }
        ...
        internal static async Task<VerificationResult> FetchVerification(string userId) 
        {
            string path;
            if (!enableTestMode) {
                path = $"real-name/v1/{TapTapAntiAddictionManager.AntiAddictionConfig.regionStr}/clients/{gameId}/users/{userId}";
            }
            else {
                path = $"real-name/v1/clients/{gameId}/users/{userId}";
            }
        ...
        internal static async Task<VerificationResult> QuickFetchVerification(string userId, AccessToken token, string key) {
            ...
            else
            {
                string path;
                if (!enableTestMode) {
                    path = $"real-name/v1/{TapTapAntiAddictionManager.AntiAddictionConfig.regionStr}/clients/{gameId}/users/{userId}/taptap-smooth";
                }
                else {
                    path = $"real-name/v1/clients/{gameId}/users/{userId}/taptap-smooth";
                }
        ...
        internal static async Task<VerificationResult> FetchVerificationManual(string json, string key, bool activelyManualVerify)
        {
            ...
            else
            {
                ...
                if (!enableTestMode) {
                    ...
                    if (activelyManualVerify)
                        path = $"real-name/v1/clients/{gameId}/users/{TapTapAntiAddictionManager.UserId}/manual";
                    else
                        path = $"real-name/v1/clients/{gameId}/users/{TapTapAntiAddictionManager.UserId}/fallback";
                }
                Dictionary<string, object> data = new Dictionary<string, object>
                {
                    ["data"] = encryptStr
