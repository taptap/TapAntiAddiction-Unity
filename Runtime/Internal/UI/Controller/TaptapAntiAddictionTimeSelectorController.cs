...
namespace TapTap.AntiAddiction.Internal {
    ...
    public class TaptapAntiAddictionTimeSelectorController : BasePanelController
    {
        ...
        protected override void OnLoadSuccess()
        {
            ...
            Year = _defaultYear;
            Month = currentDateTime.Month > DefaultMonth ? DefaultMonth : currentDateTime.Month;
            Day = currentDateTime.Day > DefaultDay ? DefaultDay : currentDateTime.Day;
        ...
        private void OnChangeYear(int year) {
            int maxMonth = 12;
            if (year == currentDateTime.Year) {
                maxMonth = currentDateTime.Month;
            }
            monthScrollView.SetItemCountFunc(() => maxMonth);
            monthScrollView.UpdateData();
            if (Month > maxMonth)
            {
                UpdateCurMonth(maxMonth);
            }
        }
        ...
        private void OnChangeYearOrMonth(int year, int month)
        {
            ...
            {
                newDayCount = currentDateTime.Day;
            }
        ...
        private async void OnConfirmButtonClicked()
        {
            ...
            if (Validate(out DateTime dateTime))
            {
                ...
                {
                    IsSending = true;
                    var verificationResult = await Verification.FetchVerificationManual(TapTapAntiAddictionManager.UserId, json, true);
                    IsSending = false;
                    Close();
            ...
            {
                UIManager.Instance.OpenToast(Config.Current.UIConfig.InputRealNameInfoVietnam.invalidateMessage, UIManager.GeneralToastLevel.Warning);
            }
        }
