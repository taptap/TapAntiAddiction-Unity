...
using TapTap.Common;
using TapTap.Common.Runtime.Internal.Utils;
using TapTap.Login;
using UnityEngine;
...
namespace TapTap.AntiAddiction
{
    public abstract class BaseAntiAddictionWorker {
        ...
        protected void ShowVerifiedToast(bool useAvatar) {
            Texture avatar = UI.UIManager.TapTapToastIcon;
            if (useAvatar) {
                TdsImage.Instance.Load(profile?.avatar, 1000, (image) => {
                    if (image != null) {
                        avatar = image;
                    }
                    var str = string.Format("您已在 TapTap 实名！");
                    UI.UIManager.Instance.OpenToast(false, str, 3, icon: avatar);
                });
            }
            else {
                var str = string.Format("您已在 TapTap 实名！");
                UI.UIManager.Instance.OpenToast(false, str, 3, icon: UI.UIManager.TapTapToastIcon);
            }
