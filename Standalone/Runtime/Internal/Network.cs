...
namespace TapTap.AntiAddiction.Internal
{
    public static class Network
    {
        ...
        /// <summary>
        /// 获取根据之前网络请求服务端返回时间计算的时间戳，单位秒，无效时，返回 0
        /// </summary>
        /// <returns></returns>
        internal static long GetCurrentServerTime()
        {
            if (HttpClient.LastServerTime <= 0)
            {
                return 0;
            }
            long startUpTime = (long)Time.realtimeSinceStartup;
            return HttpClient.LastServerTime + startUpTime - HttpClient.LastStartUpTime;
        }
    }
}
