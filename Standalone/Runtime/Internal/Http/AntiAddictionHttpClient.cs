...
namespace TapTap.AntiAddiction.Internal.Http {
    public class AntiAddictionHttpClient
    {
        ...
        // 服务端同步的时间戳
        internal long LastServerTime { get; private set; }

        // 设置服务端时间时，当前应用启动时间
        internal long LastStartUpTime { get; private set; }
            ...
            Dictionary<string, object> queryParams = null)
        {
            ...
            while (retryTimes > 0)
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (ret is BaseResponse successResponse && successResponse.Now > 0)
                    {
                        ResetLastServerTime(successResponse.Now);
                    }
                    return ret;
                }
        ...
        private void ResetLastServerTime(long serverTime)
        {
