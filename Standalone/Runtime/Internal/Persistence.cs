...
namespace TapTap.AntiAddiction.Internal 
{
    ...
    internal class Persistence 
    {
        ...
        internal Persistence(string path) 
        {
            ...
            if (!string.IsNullOrEmpty(TapTapAntiAddictionManager.config.gameId)){
                new_cacheDir = Path.Combine(Application.persistentDataPath, Config.ANTI_ADDICTION_DIR + "_" + TapTapAntiAddictionManager.config.gameId);
            }
            // 文件夹不存在时，尝试兼容旧版本数据
