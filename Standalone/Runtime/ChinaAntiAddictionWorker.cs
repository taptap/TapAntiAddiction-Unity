    ...
    public sealed class ChinaAntiAddictionWorker : BaseAntiAddictionWorker
    {
        ...
        private async Task<int> GetVerificationResult(string userId, bool justQuickVerify) {
            ...
            try {
                ...
                if (accessToken != null) {
                    Debug.LogFormat("[TapTap: ChinaAntiAddiction] 获得登录Access Token!Token结果: {0}\n是否包括Compliance: {1}",
                        accessToken.ToJson(), HaveComplianceScope(accessToken));
                    if (HaveComplianceScope(accessToken)) {
                        await GetQuickVerificationResult(userId, accessToken);
                    }
                    else {
                        //TODO@luran:本地化
                    ...
                    Debug.LogFormat("[TapTap: ChinaAntiAddiction] 不能登录Access Token!直接降级为手动认证");
                    mannualVerify = 2;
                }
            }
        ...
        private static async Task<AccessToken> GetAccessToken(AccessToken accessToken, bool justQuickVerify) {
            ...
            Profile profile = await TapLogin.GetProfile();
            ChinaAntiAddictionWorker.profile = profile;
            var isTapUser = IsTapUser(accessToken, profile);
            if (isTapUser) {
                haveCompliance = HaveComplianceScope(accessToken);
            }
            Debug.LogFormat($"[TapTap: ChinaAntiAddiction] 启动快速实名制. 是否为Tap用户: {isTapUser} 是否包含Complaince: {haveCompliance}");
        ...
        private static async Task<AccessToken> InternalGetAccessToken(bool isTapUser, bool haveCompliance, AccessToken accessToken) {
            ...
            if (!isTapUser || !haveCompliance) {
                ...
                Debug.LogFormat($"[TapTap: ChinaAntiAddiction] 因为不是Tap用户启动快速认证提示弹窗!弹窗结果: {goQuickVerify}");
                // 同意授权
                if (goQuickVerify) {
                    getToken = () =>
                        //https://xindong.slack.com/archives/C0271SPG77A/p1695810019614269?thread_ts=1695378011.212349&cid=C0271SPG77A
                        TapLogin.Authorize(new string[] { TapLogin.TAP_LOGIN_SCOPE_COMPLIANCE, TapLogin.TAP_LOGIN_SCOPE_PUBLIC_PROFILE });
                }
                //不同意授权
                else {
        ...
        private static bool IsTapUser(AccessToken accessToken, Profile profile) {
            return accessToken != null && !string.IsNullOrEmpty(accessToken.kid) && profile != null;
        }
    }
}