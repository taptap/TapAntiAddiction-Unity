...
namespace Plugins.AntiAddictionUIKit
{
    public static class AntiAddictionUIKit
    {
        ...
        [DllImport("__Internal")]
        #endif
        private static extern void TapSDKStartupWithTapTap(string userIdentifier, bool isTapUser);

        #if UNITY_IOS
        [DllImport("__Internal")]
        #endif
        private static extern void TapSDKStartupWithTapTap(string userIdentifier);
        ...
        private class NativeAntiAddictionUICallbackHandler : MonoBehaviour
        {
            ...
            {
                TapLogger.Debug("HandleAntiAddictionCallbackMsg antiAddictionCallbackDataJSON:" + antiAddictionCallbackDataJSON);
                var antiAddictionCallbackOriginData = JsonUtility.FromJson<AntiAddictionCallbackOriginData>(antiAddictionCallbackDataJSON);

                TapLogger.Debug("HandleAntiAddictionCallbackMsg resultCode:" + antiAddictionCallbackOriginData.code);
                ...
                if (antiAddictionCallbackOriginData.extras != null && antiAddictionCallbackOriginData.extras.Length > 0) {
                    ...
                    TapLogger.Debug("result.extras title:" + result.extras.title);
                    TapLogger.Debug("result.extras description:" + result.extras.description);
                }
        ...
        public static void StartupWithTapTap(string userIdentifier) {
            ...
            {
                case RuntimePlatform.Android:
                    PerformAndroidStartupWithTapTap(userIdentifier);
                    break;
                case RuntimePlatform.IPhonePlayer:
                    PerformiOSStartupWithTapTap(userIdentifier);
                    break;
                default:
                    throw new PlatformNotSupportedException();
            }
        ...
        {
            TapLogger.Debug("Android Init calling:" + gameIdentifier);
        ...
        {
            TapLogger.Debug("Android startup calling");
            AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            AndroidJavaObject unityActivity = unityClass.GetStatic<AndroidJavaObject>("currentActivity");
        ...
        {
            TapLogger.Debug("Android startup new calling");
            AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            AndroidJavaObject unityActivity = unityClass.GetStatic<AndroidJavaObject>("currentActivity");
            androidJavaNativeAntiAddiction.Call("startup", unityActivity, userIdentifier, isTapUser);
        }

        private static void PerformAndroidStartupWithTapTap(string userIdentifier)
        {
            TapLogger.Debug("Android startupWithTapTap new calling");
            AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            AndroidJavaObject unityActivity = unityClass.GetStatic<AndroidJavaObject>("currentActivity");
            androidJavaNativeAntiAddiction.Call("startupWithTapTap", unityActivity, userIdentifier);
        }

        private static string PerformAndroidGetCurrentToken()
        {
            TapLogger.Debug("Android getCurrrentToken calling");
            return androidJavaNativeAntiAddiction.Call<string>("getCurrentToken");
        }

        private static int PerformAndroidGetCurrentUserAgeLimit()
        {
            TapLogger.Debug("Android getCurrrentUserAgeLimit calling");
        ...
        private static int PerformAndroidIsStandalone()
        {
            ...
            TapLogger.Debug("Android PerformAndroidIsStandalone calling" + result.ToString());
            return result;
        }
        
        private static void PerformAndroidSetTestEnvironment(bool enable)
        {
            TapLogger.Debug("Android SetTestEnvironment calling");
            AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            AndroidJavaObject unityActivity = unityClass.GetStatic<AndroidJavaObject>("currentActivity");
        ...
        {
            TapLogger.Debug("PerformiOSInit:" + gameIdentifier);
        ...
        {
            TapLogger.Debug("PerformiOSStartup:" + userIdentifier);
        ...
        {
            TapLogger.Debug("PerformiOSStartupNew:" + userIdentifier);
            TapSDKStartupWithTapTap(userIdentifier, isTapUser);
        }

        private static void PerformiOSStartupWithTapTap(string userIdentifier)
        {
            TapLogger.Debug("PerformiOSStartupWithTapTap:" + userIdentifier);
            TapSDKStartupWithTapTap(userIdentifier);
        }

        private static string PerformiOSGetCurrentToken()
        {
            // to implement
            TapLogger.Debug("PerformiOSGetCurrentToken");
